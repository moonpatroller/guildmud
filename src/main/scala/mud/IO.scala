...
object IO
{
     ...
     * va_ functions.
     */
    def log_string(txt: String, args: Any*)(implicit dmobile_list: List[dMobile]): Unit = {
        //   FILE *fp;
        ...
        //   va_list args;
        val strtime = Utils.get_timestamp()
        val strdate = Utils.get_date()

        //   va_start(args, txt);
        //   vsnprintf(buf, MAX_BUFFER, txt, args);
        //   va_end(args);
        val buf = String.format(txt, args)

        //   /* point to the correct logfile */
        //   snprintf(logfile, MAX_BUFFER, "../log/%s.log", strdate);
        val logfile = s"./log/${strdate}.log"
        ...
        //   fprintf(fp, "%s: %s\n", strtime, buf);
        //   fclose(fp);
        val fp = new java.io.OutputStreamWriter(new java.io.FileOutputStream(logfile), "UTF-8")
        try {
            fp.append(s"${strtime}: ${buf}\n")
        }
        catch {
            case _: Exception => Utils.communicate(null, "bug: cannot open logfile", COMM_LOG)
        }
        finally {
            fp.close()
        }

        Utils.communicate(null, buf, COMM_LOG)
    }
     ...
     * va_ functions.
     */
    def bug(txt: String, args: Any*)(implicit dmobile_list: List[dMobile]): Unit = {
        // FILE *fp;
        // char buf[MAX_BUFFER];
        // va_list args;
        val strtime = Utils.get_timestamp()
        ...
        // fprintf(fp, "%s: %s\n", strtime, buf)
        // fclose(fp)
        val fp = new java.io.OutputStreamWriter(new java.io.FileOutputStream("./log/bugs.txt"), "UTF-8")
        try {
            fp.append(s"${strtime}: ${buf}\n")
        }
        catch {
            case _: Exception => Utils.communicate(null, "bug: cannot open bugfile", COMM_LOG)
        }
        finally {
            fp.close()
        }
